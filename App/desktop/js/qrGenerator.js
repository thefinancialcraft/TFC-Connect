...
function generateCheckOutQRCode() {
    const extSystemIdElement = document.getElementById('extSystemId');
    const extAsignUserElement = document.getElementById('extAsignUser');
    const qrTimeElement = document.querySelector('.qr-time-checkout'); // Select the element with class "qr-time"
    ...
    const qrCodeContainer = document.getElementById('qrCheckoutBox');

    // Clear previous QR code if exists
    qrCodeContainer.innerHTML = '';

    // Generate QR code
    QRCode.toCanvas(qrInput, { width: 150 }, (error, canvas) => {
        if (error) {
            console.error(error);
            alert('Failed to generate QR code!');
            return;
...
setInterval(sendCheckOutScanStatus, 1000);

function sendCheckOutScanStatus() {
    // Get the values of extSystemId and extAsignUser
    const extSystemIdElement = document.getElementById('extSystemId');
    const checkOutQrCon = document.getElementById('checkOutQrCon');
    const miscInfo = document.getElementById('misc-info');
    const constructionDisp = document.getElementById('construction-disp');
    ...
    fetch('/TFC-Connect/App/config.json')
        ...
        .then(config => {
            ...
            // Create data to send in the POST request
            const data = new URLSearchParams();
            data.append('action', 'checkCheckOutScanStatus');
            data.append('extSystemId', extSystemId);
            data.append('extAsignUser', extAsignUser);
        ...
        .then(data => {
            if (data) {
                ...
                // Check the response value
                if (data.data === false) {
                    checkOutQrCon.style.display = "none";
                    menuCard.style.display = "block";
                   
                } else {
                    checkOutQrCon.style.display = "flex";
                    miscInfo.style.display = "flex";
                    menuCard.style.display = "none";
                    constructionDisp.style.display = "none";
                }
            }
        })
        .catch(error => {
